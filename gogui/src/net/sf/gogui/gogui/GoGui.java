...
public class GoGui extends JFrame implements AnalyzeDialog.Listener,
		GuiBoard.Listener, GameTreeViewer.Listener, GtpShell.Listener,
		ScoreDialog.Listener, GoGuiMenuBar.Listener, ContextMenu.Listener,
		LiveGfx.Listener {
	public GoGui(String program, File file, int move, String time,
			boolean verbose, boolean initComputerColor, boolean computerBlack,
			boolean computerWhite, boolean auto, String gtpFile,
			String gtpCommand, File analyzeCommandsFile) throws GtpError,
			ErrorMessage {
		/* SORIANO: start statistics window */
		JFrame globalStatisticsFrame = new JFrame("Global Statistics");
		globalStatisticsFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		globalStatisticsFrame.setSize(800, 775);
		JPanel globalPane = new JPanel();
		globalStatisticsFrame.add(globalPane);
		globalPane.setLayout(new OverlayLayout(globalPane));
		final GlobalStatistics global = new GlobalStatistics();
		globalPane.add(global);
		fixedImage background = new fixedImage("img/backgroundSmall.JPG", 0, 0);
		globalPane.add(background);
		globalStatisticsFrame.setVisible(true);

		int delay = 1000; // milliseconds

		ActionListener taskPerformerGlobal = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				global.repaint();
			}
		};

		new Timer(delay, taskPerformerGlobal).start();

		JFrame statisticsFrame = new JFrame("Statistics");
		statisticsFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		statisticsFrame.setSize(800, 775);
		JPanel pane = new JPanel();
		statisticsFrame.add(pane);
		pane.setLayout(new OverlayLayout(pane));
		final StatisticsGraphs graphs = new StatisticsGraphs();
		pane.add(graphs);
		fixedImage background2 = new fixedImage("img/backgroundSmall.JPG", 0, 0);
		...
		m_splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, m_guiBoard,
				m_infoPanel);
		...
		m_menuBar = new GoGuiMenuBar(m_actions, recentListener, recentGtp, this);
		m_treeLabels = m_prefs.getInt("gametree-labels",
				GameTreePanel.LABEL_NUMBER);
		m_treeSize = m_prefs.getInt("gametree-size", GameTreePanel.SIZE_NORMAL);
		m_showSubtreeSizes = m_prefs.getBoolean("gametree-show-subtree-sizes",
				false);
		m_autoNumber = m_prefs.getBoolean("gtpshell-autonumber", false);
		m_commandCompletion = !m_prefs.getBoolean(
				"gtpshell-disable-completions", false);
	...
	public void actionBackward(int n) {
		if (!checkStateChangePossible())
			return;
		boolean protectGui = (m_gtp != null && (n > 1 || !m_gtp
				.isSupported("undo")));
	...
	public void actionBoardSizeOther() {
		if (!checkCommandInProgress())
			return;
		int size = BoardSizeDialog.show(this, getBoardSize(), m_messageDialogs);
	...
	public void actionDeleteSideVariations() {
		...
		if (!m_messageDialogs.showQuestion(disableKey, this,
				i18n("MSG_DELETE_VARIATIONS"), i18n("MSG_DELETE_VARIATIONS_2"),
				i18n("LB_DELETE"), false))
			return;
	...
	public void actionDetachProgram() {
		...
		if (isCommandInProgress()
				&& !showQuestion(
						format(i18n("MSG_TERMINATE_COMMAND_IN_PROGRESS"),
								getProgramLabel()),
	...
	public void actionHelp() {
		...
		if (m_help == null) {
			m_help = new Help(url, m_messageDialogs, i18n("TIT_HELP") + " - "
					+ i18n("LB_GOGUI"));
	...
	public void actionEditLabel(GoPoint point) {
		...
		value = (String) JOptionPane.showInputDialog(this, message,
				i18n("TIT_EDIT_LABEL"), JOptionPane.PLAIN_MESSAGE, null, null,
				value);
		if (value == null)
			return;
		...
		Runnable runnable = new Runnable() {
			public void run() {
				try {
					...
					ConstNode node = NodeUtil.findInComments(currentNode,
							m_pattern);
					...
					if (node == null) {
						unprotectGui();
						showInfo(
								i18n("MSG_FIND_NOT_FOUND"),
								format(i18n("MSG_FIND_NOT_FOUND_2"), m_pattern),
								false);
						m_pattern = null;
					} else {
						gotoNode(node);
	...
	public void actionGotoVariation() {
		...
		ConstNode node = GotoVariationDialog.show(this, getTree(),
				getCurrentNode(), m_messageDialogs);
		if (node == null)
			return;
		actionGotoNode(node);
	}
	...
	public void actionKeepOnlyPosition() {
		...
		if (!showQuestion(i18n("MSG_KEEP_ONLY_POSITION"),
				i18n("MSG_KEEP_ONLY_POSITION_2"), i18n("LB_DELETE"), true))
			return;
	...
	public void actionMakeMainVariation() {
		...
		if (!m_messageDialogs.showQuestion(disableKey, this,
				i18n("MSG_MAKE_MAIN_VAR"), i18n("MSG_MAKE_MAIN_VAR_2"),
	...
	public void actionNewProgram() {
		...
		m_newProgram = editor.editItem(this, i18n("TIT_NEW_PROGRAM"),
				m_newProgram, true, false, m_messageDialogs);
			...
			public void run() {
				...
				if (m_gtp == null || m_gtp.isProgramDead()) {
					m_newProgram = editor.editItem(GoGui.this,
							i18n("TIT_NEW_PROGRAM"), m_newProgram, true, false,
							m_messageDialogs);
					if (m_newProgram == null)
						return;
				...
				m_newProgram = editor.editItem(GoGui.this,
						i18n("TIT_NEW_PROGRAM"), m_newProgram, false, true,
						m_messageDialogs);
				if (m_newProgram == null) {
					actionDetachProgram();
					return;
	...
	public void actionPass() {
		if (!checkStateChangePossible())
			return;
		if (!showOptionalQuestion("pass", i18n("MSG_PASS"), i18n("MSG_PASS_2"),
				i18n("LB_PASS"), false))
			return;
	...
	public void actionPreviousEarlierVariation() {
		if (!checkStateChangePossible())
			return;
		ConstNode node = NodeUtil.getPreviousEarlierVariation(getCurrentNode());
		if (node != null)
			actionGotoNode(node);
	}
	...
	public void actionSave() {
		...
		if (m_gameFile == null)
			...
			if (file.exists()) {
				...
				if (!m_messageDialogs.showQuestion(disableKey, this,
						mainMessage, optionalMessage, i18n("LB_REPLACE_FILE"),
						true))
					return;
			}
			save(m_gameFile);
	...
	public void actionScore() {
		...
		if (m_gtp == null || !programReady) {
			...
			m_messageDialogs.showInfo(disableKey, this,
					i18n("MSG_SCORE_MANUAL"), i18n(optionalMessage), true);
			updateViews(false);
			initScore(null);
			return;
		}
		if (m_gtp.isSupported("final_status_list")) {
			...
			m_messageDialogs.showInfo(disableKey, this,
					i18n("MSG_SCORE_MANUAL"), optionalMessage, true);
			updateViews(false);
			initScore(null);
		}
	}
	...
	public void actionSetAnalyzeCommand(AnalyzeCommand command,
			boolean autoRun, boolean clearBoard, boolean oneRunOnly) {
		if (!checkProgramReady())
			return;
		if (!checkStateChangePossible())
			return;
		...
		if (needsPointArg && !m_analyzeCommand.isPointArgMissing()) {
			...
			if (m_analyzeCommand.getType() == AnalyzeType.EPLIST)
				GuiBoardUtil.setSelect(m_guiBoard,
						m_analyzeCommand.getPointListArg(), true);
			toFront();
			return;
		}
		analyzeBegin(false);
	...
	public void actionSwitchLanguage() {
		SwitchLanguageDialog dialog = new SwitchLanguageDialog(this,
				m_messageDialogs, m_prefs, "language");
	...
	public void actionToggleCompletion() {
		...
		m_prefs.putBoolean("gtpshell-disable-completions", !m_commandCompletion);
	}
	...
	public void actionTruncate() {
		...
		if (!m_messageDialogs.showQuestion(disableKey, this,
				i18n("MSG_TRUNCATE"), i18n("MSG_TRUNCATE_2"),
	...
	public void actionTruncateChildren() {
		...
		if (!m_messageDialogs.showQuestion(disableKey, this,
				i18n("MSG_TRUNCATE_CHILDREN"), i18n("MSG_TRUNCATE_CHILDREN_2"),
				i18n("LB_TRUNCATE"), false))
			return;
	 ...
	 *         Program instance, otherwise label as in GtpClientBase#getLabel;
	 *         null if no program is attached.
	 */
	...
	public void contextMenu(GoPoint point, Component invoker, int x, int y) {
		if (m_setupMode
				|| (m_analyzeCommand != null && m_analyzeCommand
						.needsPointListArg())) {
	...
	public void fieldClicked(GoPoint p, boolean modifiedSelect) {
		if (m_setupMode) {
			...
			if (getBoard().isSuicide(getToMove(), p)
					&& !showQuestion(i18n("MSG_SUICIDE"),
							i18n("MSG_SUICIDE_2"), i18n("LB_SUICIDE"), false))
				return;
			else if (getBoard().isKo(p)
					&& !showQuestion(i18n("MSG_ILLEGAL_KO"),
							i18n("MSG_ILLEGAL_KO_2"), i18n("LB_ILLEGAL_KO"),
							false))
				return;
	...
	/**
	 * Callback for selected text. This is a callback for text selection exents
	 * in different components. It parses the text for valid points and marks
	 * them on the board.
	 */
	public void textSelected(String text) {
		if (text == null)
	...
	private class AnalyzeContinue implements Runnable {
	...
	private class ShowInvalidResponse implements Runnable {
		...
		public void run() {
			...
			/*
			 * if (m_line.trim().equals("")) { String disableKey =
			 * "net.sf.gogui.gogui.GoGui.invalid-empty-response"; String
			 * optionalMessage = format(i18n("MSG_INVALID_EMPTY_RESPONSE"),
			 * name); m_messageDialogs.showWarning(disableKey, GoGui.this,
			 * mainMessage, optionalMessage, true); } else { String disableKey =
			 * "net.sf.gogui.gogui.GoGui.invalid-response"; String
			 * optionalMessage = format(i18n("MSG_INVALID_NOSTATUS_RESPONSE"),
			 * name); m_messageDialogs.showWarning(disableKey, GoGui.this,
			 * mainMessage, optionalMessage, true); }
			 */// ** Removed temporarily
		}
	...
	private static class LoadFileRunnable implements GuiUtil.ProgressRunnable {
	...
	/**
	 * State variable used between generateMove and computerMoved. Flag is set
	 * in actionInterrupt.
	 */
	...
	private final Preferences m_prefs = Preferences
	...
	/**
	 * Program information. Can be null even if a program is attached, if only
	 * m_programName is known.
	 */
	...
	private final ThumbnailCreator m_thumbnailCreator = new ThumbnailCreator(
			false);
	...
	private void analyzeBegin(boolean checkComputerMove) {
		...
		runLengthyCommand(m_lastAnalyzeCommand, new AnalyzeContinue(
				checkComputerMove));
		...
		try {
			...
			if (type == AnalyzeType.PARAM)
				ParameterDialog.editParameters(m_lastAnalyzeCommand, this,
						title, response, m_gtp, m_messageDialogs);
	 ...
	 * Attach program.
	 * 
	 * @param programCommand
	 *            Command line for running program.
	 * @param program
	...
	private boolean attachProgram(String programCommand, Program program) {
		...
		GtpClient.IOCallback ioCallback = new GtpClient.IOCallback() {
			...
			public void receivedStdErr(String s) {
				...
				while (m_lineReader.hasLines()) {
					...
					boolean isWarning = line.startsWith("warning:")
		...
		try {
			...
			GtpClient gtp = new GtpClient(m_programCommand, workingDirectory,
			...
			try {
				String programAnalyzeCommands = GtpClientUtil
						.getAnalyzeCommands(m_gtp);
				m_analyzeCommands = AnalyzeDefinition.read(
						m_gtp.getSupportedCommands(), m_analyzeCommandsFile,
			...
			ArrayList<String> supportedCommands = m_gtp.getSupportedCommands();
	...
	private void boardChangedBegin(boolean doCheckComputerMove,
		...
		if (m_gtp != null && !isOutOfSync() && m_analyzeCommand != null
				&& m_analyzeAutoRun && !m_analyzeCommand.isPointArgMissing())
	...
	private boolean checkHasParameterCommands() {
		if (!AnalyzeUtil.hasParameterCommands(m_analyzeCommands)) {
			String optionalMessage = format(i18n("MSG_NO_PARAM_COMMANDS_2"),
					getProgramName());
	...
	private boolean checkProgramReady() {
		...
		if (m_gtp.isProgramDead()) {
			...
			if (m_shell.isLastTextNonGTP()) {
				showShell();
				optionalMessage = format(
						i18n("MSG_PROGRAM_TERMINATED_CHECK_GTP"), name);
			} else {
				showShell();
				optionalMessage = format(
						i18n("MSG_PROGRAM_TERMINATED_REATTACH"), name);
			}
	 ...
	 * 
	 * @return true If file was not modified, user chose not to save it or file
	...
	private boolean checkSaveGame(boolean isProgramTerminating) {
		...
		result = m_messageDialogs.showYesNoCancelQuestion(disableKey, this,
				mainMessage, optionalMessage, i18n("LB_DONT_SAVE"),
				i18n("LB_SAVE"));
		switch (result) {
		case 0:
	...
	private void computerMoved() {
		...
		try {
			...
			if (response.equalsIgnoreCase("resign")) {
				String result = toMove.otherColor().getUppercaseLetter()
						+ "+Resign";
				if (!m_auto) {
					String mainMessage = format(i18n("MSG_RESIGN"), name);
					String optionalMessage = format(i18n("MSG_RESIGN_2"),
							result);
					showInfo(mainMessage, optionalMessage, false);
				}
				m_resigned = true;
				...
				if (point != null) {
					if (board.getColor(point) != EMPTY) {
						String mainMessage = format(i18n("MSG_NONEMPTY"), name);
						String optionalMessage = format(i18n("MSG_NONEMPTY_2"),
								name);
						showWarning(mainMessage, optionalMessage, true);
						m_computerBlack = false;
						m_computerWhite = false;
					} else if (board.isKo(point)) {
						String mainMessage = format(i18n("MSG_VIOLATE_KO"),
								name);
						showWarning(mainMessage, i18n("MSG_VIOLATE_KO_2"), true);
						m_computerBlack = false;
						m_computerWhite = false;
					}
				}
				...
				if (point == null && !isComputerBoth()) {
					...
					String mainMessage = format(i18n("MSG_PROGRAM_PASS"), name);
					String optionalMessage = format(i18n("MSG_PROGRAM_PASS_2"),
							name);
			...
			boolean doCheckComputerMove = (!m_isSingleMove && !(isComputerBoth() && m_interruptComputerBoth));
	...
	private ContextMenu createContextMenu(GoPoint point) {
		...
		return new ContextMenu(point, noProgram, m_analyzeCommands,
				m_guiBoard.getMark(point), m_guiBoard.getMarkCircle(point),
				m_guiBoard.getMarkSquare(point),
				m_guiBoard.getMarkTriangle(point), this);
	}
	...
	private void currentNodeChanged() {
		...
		try {
			...
			if (!wasOutOfSync) {
				...
				String optionalMessage;
				if (command == null)
					optionalMessage = format(i18n("MSG_NOSYNC_ERROR"), name,
							message);
				else if (response == null)
					optionalMessage = format(i18n("MSG_NOSYNC_FAILURE"),
							command, name);
				else
					optionalMessage = format(
							i18n("MSG_NOSYNC_FAILURE_RESPONSE"), command, name,
							response);
				showWarning(mainMessage, optionalMessage, true);
			}
		}
	...
	private void generateMove(boolean isSingleMove) {
		...
		String playerOther = info
				.get(StringInfoColor.NAME, toMove.otherColor());
		String name = getProgramLabel();
		if (!isSingleMove
				&& m_gameFile == null
				&& playerToMove == null
				&& (father == null || (!father.hasFather() && (playerOther == null || playerOther
						.equals(name))))) {
	...
	/**
	 * Change current node. Automatically restores the clock, or halts it, if no
	 * time settings are known.
	 */
	...
	private void importTextPosition(Reader reader) {
		try {
			...
			parser.parse(reader);
			GameTree tree = NodeUtil.makeTreeFromPosition(null,
					parser.getBoard());
			m_game.init(tree);
		} catch (ParseError e) {
	...
	private void initGame(int size) {
		...
		if (handicap == null)
			showWarning(i18n("MSG_HANDICAP_UNDEFINED"),
					format(i18n("MSG_HANDICAP_UNDEFINED_2"), m_handicap, size),
					false);
	...
	private boolean loadFile(File file, int move) {
		try {
			...
			initGtp();
			if (move > 0) {
				ConstNode node = NodeUtil.findByMoveNumber(getCurrentNode(),
						move);
				if (node != null)
					m_game.gotoNode(node);
			}
			...
			if (warnings != null) {
				String optionalMessage = i18n("MSG_FILE_FORMAT_WARNING_2")
						+ "\n("
						+ warnings.replaceAll("\n\\z", ")").replaceAll("\n",
								")\n(") + ")";
	...
	/**
	 * Paint point immediately to pretend better responsiveness. Necessary
	 * because waiting for a repaint of the Go board can be slow due to the
	 * updating game tree or response to GTP commands.
	 */
	...
	private void restoreBoardCursor() {
		if (m_analyzeCommand != null
				&& (m_analyzeCommand.needsPointArg() || m_analyzeCommand
						.needsPointListArg()))
			setBoardCursor(Cursor.HAND_CURSOR);
		else
			setBoardCursorDefault();
	...
	private void restoreMainWindow(int size) {
		...
		if (fieldSize > 0)
			m_guiBoard
					.setPreferredFieldSize(new Dimension(fieldSize, fieldSize));
	...
	private boolean save(GameFile gameFile) {
		try {
			new GameWriter(gameFile, getTree(), i18n("LB_GOGUI"), Version.get());
		} catch (ErrorMessage e) {
	...
	private void setMinimumSize() {
		...
		if (m_showToolbar) {
			...
			width = Math.max(width, (int) toolBarSize.getWidth()
					+ rootInsetsWidth + contentInsetsWidth + GuiUtil.PAD);
	...
	private void setResult(String result) {
		...
		if (!(oldResult == null || oldResult.equals("") || oldResult
				.equals(result))
				&& !showQuestion(
						format(i18n("MSG_REPLACE_RESULT"), oldResult, result),
	...
	private void setTitle() {
		...
		if (gameName == null)
			...
			if (!appName.equals(i18n("LB_GOGUI"))
					&& (ObjectUtil.equals(nameBlack, name) || ObjectUtil
							.equals(nameWhite, name)))
				setTitle(gameName);
			else
	...
	private void showError(GtpResponseFormatError e) {
		String name = getProgramName();
		String mainMessage = format(i18n("MSG_INVALID_RESPONSE"), name);
		String optionalMessage = format(i18n("MSG_INVALID_RESPONSE_2"), name,
				e.getMessage());
	...
	private void showError(GtpError e, boolean isCritical) {
		...
		if (m_gtp != null && m_gtp.isProgramDead()) {
			...
			if (hasErrorOutput && !anyResponses)
				optionalMessage = format(i18n("MSG_PROGRAM_TERMINATED_2"), name);
			else if (hasErrorOutput && anyResponses)
				optionalMessage = format(i18n("MSG_PROGRAM_TERMINATED_3"), name);
			else
			...
			if (StringUtil.isEmpty(e.getMessage()))
				...
				optionalMessage = format(i18n("MSG_COULD_NOT_EXECUTE_3"),
						e.getMessage());
		} else {
			mainMessage = i18n("MSG_COMMAND_FAILED");
			if (e.getMessage().trim().equals(""))
				optionalMessage = format(i18n("MSG_COMMAND_FAILED_2"),
						e.getCommand());
			else
				optionalMessage = format(i18n("MSG_COMMAND_FAILED_3"),
						e.getCommand(), e.getMessage());
		}
	...
	private void showGameFinished() {
		...
		m_messageDialogs.showInfo(disableKey, this, i18n("MSG_GAME_FINISHED"),
	...
	private boolean showOptionalQuestion(String id, String mainMessage,
			String optionalMessage, String destructiveOption, boolean isCritical) {
		String disableKey = "net.sf.gogui.gogui.GoGui" + id;
		return m_messageDialogs.showQuestion(disableKey, this, mainMessage,
				optionalMessage, destructiveOption, isCritical);
	}
	 ...
	 * Update all views.
	 * 
	 * @param gameTreeChanged
	 *            If nodes were added to or removed from the game tree, which
	 *            will trigger a full and potentially slow game tree update
	 * @param sync
	 *            Update game tree within the event handler if the gameTree has
	 *            changed.
	 */
	...
	private void updateGuiBoard() {
		if (m_showVariations) {
			ConstPointList childrenMoves = NodeUtil
					.getChildrenMoves(getCurrentNode());
